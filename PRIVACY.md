...
- Not App requests the following permissions:
  - Permission to list installed apps for the app-picker (a non-runtime,
    non-sensitive query).
  - Permission to display over other apps (SYSTEM_ALERT_WINDOW) — needed to show
    the settings shortcut overlay when the config panel is hidden, ensuring you
    can always access launcher settings.
